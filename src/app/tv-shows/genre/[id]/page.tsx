...
export default async function GenreTvShowPage({
    ...
    const { id } = params;
    const genreData = await getData(`/genre/tv/list?language=en`);
    const genreName = genreData.genres.find((genre: any) => genre.id == id).name;
    const tv = await getData(`/discover/tv?include_adult=false&include_video=false&language=en-US&page=1&sort_by=popularity.desc&with_genres=${id}`);
    return (
        <>
